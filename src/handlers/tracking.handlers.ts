...
export const getOneTracking = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
    res.json({
      Message: ` the tracking was retrieved successfully`,
      data: { ...ts }
    })
  } catch (err) {
    next(err)
...
  try {
    const t = await trackingModel.deleteTracking(req.params.id as unknown as string)
    res.json({
